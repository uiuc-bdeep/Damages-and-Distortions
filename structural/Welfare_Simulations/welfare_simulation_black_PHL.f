...
c
      ...
      PARAMETER (np1=10000,np=30000,nchoice=138,np3=5*np)
      PARAMETER (hinc=140.d0)
...
c
      OPEN(20,file='../cleaned_data/tracts_PHL.txt')
      REWIND(20)
      DO j=1,nchoice
     ...
     &      z1(45,j),z1(46,j),z1(47,j)  
      ...
      enddo
      open(25,file='../cleaned_data/latlon_PHL.txt')
      rewind(25)
      do j = 1,nchoice
         read(25,*) lon(j),lat(j)
      enddo
      open(30,file='../probabilities_2/fitprob3.txt')
      rewind(30)
      do j=1,nchoice
...
c
     ...
     &                b(1)*dlog(inc(j)-x(1,k)*12.d0)
               ...
               enddo
               util(k) = util(k)+b(nparam1+5)*lat(k)+
     &                   b(nparam1+6)*lon(k)
 299           continue               
            enddo
            denom = 0.d0
...
c      
      ...
      enddo
c
      open(500,file='simres_phl_black.csv')
      rewind(500)
